...
export default function MeltingLotPage() {
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to delete melting lot';
      showErrorToastUnlessAuth(msg);
    }
  ...
  const actions: TableAction<EntityRow>[] = useMemo(() => {
    // Build actions array dynamically based on row status
    return [
      {
        label: 'Edit',
        onClick: (row: EntityRow) => {
          const rowStatus = row.status;
          if (rowStatus === 'Submitted') return; // Don't navigate if submitted
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(deleteConfirmRow.product ?? deleteConfirmRow.id ?? 'this melting lot')
      : '';
  ...
  return (
    <>
      ...
      <ListPageLayout
        ...
        toolbarLeft={
          <h2
            className={`flex items-center gap-2 text-xl font-bold tracking-tight ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
              }`}
          >

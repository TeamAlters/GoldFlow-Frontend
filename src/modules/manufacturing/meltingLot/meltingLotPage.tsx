...
export default function MeltingLotPage() {
  ...
  // Show Edit/Delete only when status is not Submitted (case-insensitive)
  const isRowSubmitted = (row: EntityRow) =>
    String(row.status ?? '').trim().toLowerCase() === 'submitted';
  ...
  const actions: TableAction<EntityRow>[] = useMemo(
    () => [
      {
        ...
        shouldShow: (row: EntityRow) => !isRowSubmitted(row),
      },
      {
        label: 'Delete',
    ...
    [idField, navigate, entityConfig.routes.edit, handleDeleteClick, deletingId, isRowSubmitted]
  );
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(deleteConfirmRow.product ?? deleteConfirmRow[idField] ?? 'this melting lot')
      : '';

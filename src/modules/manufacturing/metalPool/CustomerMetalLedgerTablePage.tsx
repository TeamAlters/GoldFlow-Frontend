    ...
    <div className="w-full">
      <div className="mb-6">
        <Breadcrumbs
          items={[
            ...
            { label: 'Metal Pool', href: '/metal-pool' },
            { label: 'Balance', href: '/metal-pool/balance' },
          ]}
          className="mb-4"
        />
        <h1
      ...
      >
        <div className="space-y-6">
          ...
          <div
            className={`overflow-hidden rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-300 bg-white'
              }`}
          >
                ...
                <thead>
                  ...
                  >
                    {['Purity', 'Balance Weight', 'Total Fine Weight'].map((h) => (
                      <th
                        key={h}
                        className={`px-4 py-2 text-xs font-bold uppercase tracking-wider whitespace-nowrap border-r ${isDarkMode ? 'border-gray-600 text-gray-300' : 'border-gray-200 text-gray-800'
                          }`}
                      >
                        {h}
                      </th>
                ...
                >
                  {loading ? (
                    <tr>
                      <td
                        colSpan={3}
                        className={`px-4 py-4 text-center text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'
                          }`}
                      >
                    ...
                    rows.map((row, index) => (
                      ...
                      >
                        <td
                          className={`px-6 py-3 text-center whitespace-nowrap border-r ${isDarkMode ? 'border-gray-500 text-white' : 'border-gray-300'
                            }`}
                        >
                          {row.purity}
                        </td>

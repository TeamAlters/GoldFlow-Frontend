...
let purityRangeMetadataFetchInFlight = false;

export default function PurityRangePage() {
  const navigate = useNavigate();
  const isDarkMode = useUIStore((state) => state.isDarkMode);
  const entityName = 'purity_range';
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (purityRangeMetadataFetchInFlight) return;
    purityRangeMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Purity Ranges',
      ...
      .finally(() => {
        purityRangeMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to delete purity range';
      showErrorToastUnlessAuth(msg);
    }

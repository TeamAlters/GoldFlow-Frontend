...
let designMetadataFetchInFlight = false;

export default function DesignsPage() {
  const navigate = useNavigate();
  const isDarkMode = useUIStore((state) => state.isDarkMode);
  const entityName = 'design';
  ...
  const [designs, setDesigns] = useState<EntityRow[]>([]);
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (designMetadataFetchInFlight) return;
    designMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Designs',
      ...
      .finally(() => {
        designMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const columns: TableColumn<EntityRow>[] = useMemo(() => {
    ...
    const idField = entityMetadata?.id_field ?? 'design_name';
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(deleteConfirmRow.design_name ?? deleteConfirmRow.name ?? 'this design')
      : '';

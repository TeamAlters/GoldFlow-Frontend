...
let itemMetadataFetchInFlight = false;
...
export default function ItemPage() {
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (itemMetadataFetchInFlight) return;
    itemMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Items',
      ...
      .finally(() => {
        itemMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const columns: TableColumn<EntityRow>[] = useMemo(() => {
    ...
    const idField = entityMetadata?.id_field ?? 'item_name';
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(deleteConfirmRow[idField] ?? deleteConfirmRow.name ?? 'this item')
      : '';

...
export default function ItemEditPage() {
  ...
  const [initialData, setInitialData] = useState<Partial<StaticItemFormData> | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    ...
    getEntity(ENTITY_NAME, decodeURIComponent(id))
      ...
      .catch((err) => {
        const msg = err instanceof Error ? err.message : 'Failed to load item name';
  ...
  const handleSubmit = useCallback(
    async (formData: StaticItemFormData) => {
      ...
      try {
        const res = await updateEntity(ENTITY_NAME, decodeURIComponent(id), toItemPayload(formData));

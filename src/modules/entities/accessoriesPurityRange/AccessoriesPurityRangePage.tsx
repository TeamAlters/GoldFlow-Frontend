...
let accessoriesPurityRangeMetadataFetchInFlight = false;

export default function AccessoriesPurityRangePage() {
  const navigate = useNavigate();
  const isDarkMode = useUIStore((state) => state.isDarkMode);
  const entityName = 'accessories_purity_range';
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (accessoriesPurityRangeMetadataFetchInFlight) return;
    accessoriesPurityRangeMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Accessories Purity Ranges',
      ...
      .finally(() => {
        accessoriesPurityRangeMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to delete accessories purity range';
      showErrorToastUnlessAuth(msg);
    }
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(deleteConfirmRow[idField] ?? deleteConfirmRow.name ?? 'this accessories purity range')
      : '';

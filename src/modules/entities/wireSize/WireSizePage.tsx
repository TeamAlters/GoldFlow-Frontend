...
let wireSizeMetadataFetchInFlight = false;
...
export default function WireSizePage() {
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (wireSizeMetadataFetchInFlight) return;
    wireSizeMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Wire Sizes',
      ...
      .finally(() => {
        wireSizeMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to delete wire size';
      showErrorToastUnlessAuth(msg);
    }

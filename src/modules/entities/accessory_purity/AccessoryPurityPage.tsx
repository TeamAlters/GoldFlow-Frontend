...
let accessoryPurityMetadataFetchInFlight = false;

export default function AccessoryPurityPage() {
  const navigate = useNavigate();
  const isDarkMode = useUIStore((state) => state.isDarkMode);
  const entityName = 'accessory_purity';
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (accessoryPurityMetadataFetchInFlight) return;
    accessoryPurityMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Accessory Purities',
      ...
      .finally(() => {
        accessoryPurityMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const columns: TableColumn<EntityRow>[] = useMemo(() => {
    ...
    const idField = entityMetadata?.id_field ?? 'accessory_purity';
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to delete accessory purity';
      showErrorToastUnlessAuth(msg);
    }
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(deleteConfirmRow[idField] ?? deleteConfirmRow.name ?? 'this accessory purity')
      : '';

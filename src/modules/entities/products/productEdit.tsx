    ...
    const handleSubmit = useCallback(
        async (formData: StaticProductFormData) => {
            ...
            try {
                ...
                const res = await updateEntity(ENTITY_NAME, decodeURIComponent(id), toProductPayload(formData));
                toast.success(`${entityConfig.displayName} updated successfully.`);
                const newId = getRedirectIdAfterUpdate(res, formData as Record<string, unknown>, id ?? '', ['product_name', 'id']);

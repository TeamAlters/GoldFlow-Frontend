...
let purityMetadataFetchInFlight = false;

export default function PuritiesPage() {
  const navigate = useNavigate();
  const isDarkMode = useUIStore((state) => state.isDarkMode);
  const entityName = 'purity';
  ...
  const [purities, setPurities] = useState<EntityRow[]>([]);
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (purityMetadataFetchInFlight) return;
    purityMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Purities',
  ...
  const fetchList = useCallback(() => {
    ...
    getEntityList(entityName, { page, page_size: pageSize, filters: filtersForApi })
      .then((res) => {
        ...
        setPurities(items as EntityRow[]);

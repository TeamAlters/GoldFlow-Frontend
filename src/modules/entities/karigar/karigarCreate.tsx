...
export function toInitialKarigarData(
  entity: Record<string, unknown>
): Partial<StaticKarigarFormData> {
  return {
    karigar: entity.karigar != null ? String(entity.karigar) : '',
  };
}

export function toKarigarPayload(data: StaticKarigarFormData): Record<string, unknown> {
  return {
    karigar: data.karigar.trim(),
  };
}
...
export default function KarigarCreatePage() {
  ...
  const handleSubmit = useCallback(
    async (formData: StaticKarigarFormData) => {
      ...
      try {
        const payload = toKarigarPayload(formData);
        const res = await createEntity(ENTITY_NAME, payload);
        toast.success(`${entityConfig.displayName} created successfully.`);
        const id = getCreatedEntityId(res, payload as Record<string, unknown>, ['karigar', 'id']);

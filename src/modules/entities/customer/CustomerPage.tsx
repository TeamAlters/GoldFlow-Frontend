...
export default function CustomerPage() {
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (customerMasterMetadataFetchInFlight) return;
    customerMasterMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      ...
      .finally(() => {
        customerMasterMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const getRowId = useCallback(
    (row: EntityRow) => row[idField] ?? row['id'] ?? row['customer_name'],
    [idField]
  );

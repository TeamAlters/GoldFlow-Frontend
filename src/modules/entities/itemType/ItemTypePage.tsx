...
let itemTypeMetadataFetchInFlight = false;

export default function ItemTypePage() {
  const navigate = useNavigate();
  const isDarkMode = useUIStore((state) => state.isDarkMode);
  const entityName = 'item_type';
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (itemTypeMetadataFetchInFlight) return;
    itemTypeMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Item Types',
      ...
      .finally(() => {
        itemTypeMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const columns: TableColumn<EntityRow>[] = useMemo(() => {
    ...
    const idField = entityMetadata?.id_field ?? 'item_type';
          ...
          return (
            <button
              ...
              onClick={(e) => {
                e.stopPropagation();
                navigate(
                  entityConfig.routes.detail.replace(':id', encodeURIComponent(String(rowId)))
                );
              }}
              className={
                isDarkMode
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to delete item type';
      showErrorToastUnlessAuth(msg);
    }
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(deleteConfirmRow[idField] ?? deleteConfirmRow.name ?? 'this item type')
      : '';

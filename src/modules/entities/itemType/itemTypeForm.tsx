...
export interface StaticItemTypeFormRef {
  getData: () => StaticItemTypeFormData;
  validate: () => boolean;
}

export interface StaticItemTypeFormProps {
  initialData?: Partial<StaticItemTypeFormData>;
  onSubmit?: (data: StaticItemTypeFormData) => void;
  onCancel?: () => void;
  isEdit?: boolean;
  readOnly?: boolean;
...
const emptyForm: StaticItemTypeFormData = {
  item_type: '',
};

const StaticItemTypeFormInner = forwardRef<StaticItemTypeFormRef, StaticItemTypeFormProps>(
  function StaticItemTypeFormInner(
    {
      initialData,
      onSubmit,
      onCancel,
    ...
    const [formData, setFormData] = useState<StaticItemTypeFormData>({ ...emptyForm });
    ...
    const handleChange = (key: keyof StaticItemTypeFormData, value: string) => {
    ...
    const validate = (): boolean => {
      ...
      if (!value) next.item_type = 'Item type is required';
        ...
        next.item_type = maxLengthError('Item type');
      setErrors(next);
    ...
    const actions =
      ...
      !readOnly && (
        <div className="flex items-center justify-end gap-3 pt-4 mt-6">
          ...
          >
            {submitLoading ? 'Saving...' : isEdit ? 'Update Item Type' : 'Create Item Type'}
          </button>
        </div>
      );
...
export default StaticItemTypeFormInner;

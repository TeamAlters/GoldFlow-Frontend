...
export function toDepartmentPayload(data: StaticDepartmentFormData): Record<string, unknown> {
  return {
...
export default function DepartmentCreatePage() {
  ...
  const handleSubmit = useCallback(
    async (formData: StaticDepartmentFormData) => {
      ...
      try {
        const payload = toDepartmentPayload(formData);
        ...
        navigate(
          id != null ? entityConfig.routes.detail.replace(':id', encodeURIComponent(String(id))) : entityConfig.routes.list
        );
      } catch (err) {

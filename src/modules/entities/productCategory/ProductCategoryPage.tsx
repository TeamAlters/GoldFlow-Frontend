...
/**
 * Product Categories list page. Uses the same pattern as ProductsPage:
 * - GET listing-metadata for columns, filters, id_field, detail_link_field
 * - GET list for table rows
 */
type EntityRow = Record<string, unknown>;

let productCategoryMetadataFetchInFlight = false;
...
export default function ProductCategoryPage() {
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (productCategoryMetadataFetchInFlight) return;
    productCategoryMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Product Categories',
      ...
      .finally(() => {
        productCategoryMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const columns: TableColumn<EntityRow>[] = useMemo(() => {
    ...
    const idField = entityMetadata?.id_field ?? 'product_category';
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Failed to delete product category';
      showErrorToastUnlessAuth(msg);
    }
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(
          (deleteConfirmRow as Record<string, unknown>)['product_category'] ??

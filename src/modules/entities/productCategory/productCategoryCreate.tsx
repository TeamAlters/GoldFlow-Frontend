...
export function toInitialProductCategoryData(
  entity: Record<string, unknown>
): Partial<StaticProductCategoryFormData> {
  return {
    product_category: entity.product_category != null ? String(entity.product_category) : '',
    product_name: entity.product_name != null ? String(entity.product_name) : '',
  };
}

export function toProductCategoryPayload(
  data: StaticProductCategoryFormData
): Record<string, unknown> {
  return {
    product_category: data.product_category.trim(),
...
export default function ProductCategoryCreatePage() {
  ...
  const handleSubmit = useCallback(
    async (formData: StaticProductCategoryFormData) => {
      ...
      try {
        const payload = toProductCategoryPayload(formData);
        const res = await createEntity(ENTITY_NAME, payload);
        toast.success(`${entityConfig.displayName} created successfully.`);
        const id = getCreatedEntityId(res, payload as Record<string, unknown>, ['product_category', 'id']);

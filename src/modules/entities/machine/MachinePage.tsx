...
let machineMetadataFetchInFlight = false;

export default function MachinePage() {
  const navigate = useNavigate();
  const isDarkMode = useUIStore((state) => state.isDarkMode);
  const entityName = 'machine';
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (machineMetadataFetchInFlight) return;
    machineMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Machines',
      ...
      .finally(() => {
        machineMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const columns: TableColumn<EntityRow>[] = useMemo(() => {
    ...
    const idField = entityMetadata?.id_field ?? 'machine_name';
  ...
  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(deleteConfirmRow.machine_name ?? deleteConfirmRow.name ?? 'this machine')
      : '';

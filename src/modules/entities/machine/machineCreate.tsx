...
export function toInitialMachineData(
  entity: Record<string, unknown>
): Partial<StaticMachineFormData> {
  return {
    machine_name: entity.machine_name != null ? String(entity.machine_name) : '',
    product_name: entity.product_name != null ? String(entity.product_name) : '',
  };
}

export function toMachinePayload(data: StaticMachineFormData): Record<string, unknown> {
  return {
...
export default function MachineCreatePage() {
  ...
  const formRef = useRef<StaticMachineFormRef>(null);
  ...
  const handleSubmit = useCallback(
    async (formData: StaticMachineFormData) => {
      setSubmitLoading(true);
      try {
        const payload = toMachinePayload(formData);
        const res = await createEntity(ENTITY_NAME, payload);
        toast.success(`${entityConfig.displayName} created successfully.`);
        const id = getCreatedEntityId(res, payload as Record<string, unknown>, ['machine_name', 'id']);
    ...
    <div className="w-full">
      ...
      <div className="mb-6">
        ...
        <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          Create a new machine linked to a product.
        </p>
      </div>
      <form

...
import type { EntityField, EntityFilterField } from '../../modules/admin/admin.api';
...
let roleMetadataFetchInFlight = false;
...
export default function RoleAndPermissionPage() {
  ...
  const fetchMetadata = useCallback(() => {
    ...
    if (roleMetadataFetchInFlight) return;
    roleMetadataFetchInFlight = true;
    ...
    getEntityMetadata(entityName)
      .then((res) => {
        ...
        if (data && (data.fields?.length || data.display_name != null || data.filters)) {
          const meta = {
            display_name: data.display_name ?? 'Roles',
      ...
      .finally(() => {
        roleMetadataFetchInFlight = false;
        setMetadataLoading(false);
      });
  ...
  const fetchList = useCallback(() => {
    ...
    getEntityList(entityName, { page, page_size: pageSize, filters: filtersForApi })
      .then((res) => {
        ...
        const itemsList = Array.isArray(inner?.items) ? inner.items : [];
  ...
  const columns: TableColumn<EntityRow>[] = useMemo(() => {
    const visibleFields =
      (entityMetadata?.fields?.filter((f) => f.visible_in_list) ?? []).filter(
        (f) => (f.name || (f as { field?: string }).field) !== 'description'
      );
    const idField = entityMetadata?.id_field ?? 'id';
    const detailLinkField = entityMetadata?.detail_link_field;

    const getRowId = (row: EntityRow) => row[idField] ?? row['id'] ?? row['name'];
  ...
  const getRowId = useCallback(
    (row: EntityRow) => row[idField] ?? row['id'] ?? row['name'],
    [idField]
  );
  ...
  const hasFilters =
    ...
    (filterConfig.addable && Object.keys(filterConfig.addable).length > 0);

  const deleteDisplayName =
    deleteConfirmRow != null
      ? String(
          (deleteConfirmRow as Record<string, unknown>)['name'] ??
            deleteConfirmRow['role_name'] ??
            'this role'
        )
      : '';
    ...
    <>
      ...
      <ListPageLayout
        title={
          metadataLoading
            ? '...'
            : (entityMetadata?.display_name ?? 'Roles & Permissions')
        }
        description="Manage roles and table-level permissions per entity."
        toolbarLeft={
          <h2
        ...
        toolbarRight={
          <button
            type="button"
            className={`w-full sm:w-auto px-3 py-1.5 text-xs font-medium rounded-lg transition-colors flex items-center justify-center gap-1.5 ${isDarkMode ? 'bg-blue-600 hover:bg-blue-700 text-white' : 'bg-blue-500 hover:bg-blue-600 text-white'}`}

...
export default function SignUp() {
    ...
    getRegistrationStatus()
      .then((data) => {
        if (!data.registration_allowed) {
          ...
          toast.error(
            'Registration is currently disabled. Please contact an administrator to create an account.'
          );
  ...
  if (registrationCheck === 'checking') {
    return (
      <div
        className={`min-h-screen flex items-center justify-center ${isDarkMode ? 'bg-slate-900' : 'bg-white'}`}
      >
        <div className="flex flex-col items-center justify-center py-16 px-4">
          <div
            className={`w-12 h-12 rounded-full border-3 border-t-transparent animate-spin ${
              isDarkMode
                ? 'border-slate-600 border-t-amber-400'
                : 'border-gray-300 border-t-amber-500'
            }`}
            style={{ borderWidth: '3px' }}
          />
          <p
            className={`mt-6 text-base font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}
          >

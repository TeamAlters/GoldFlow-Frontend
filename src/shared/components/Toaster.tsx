...
export default function Toaster() {
  ...
  const timersRef = useRef<Map<string, { auto: ReturnType<typeof setTimeout>; exit: ReturnType<typeof setTimeout> }>>(new Map())

  const startExit = (id: string) => {
    if (exitingIds.has(id)) return
    const existing = timersRef.current.get(id)
    if (existing?.auto) clearTimeout(existing.auto)
    timersRef.current.set(id, { ...existing, auto: undefined! } as { auto: ReturnType<typeof setTimeout>; exit: ReturnType<typeof setTimeout> })
    ...
    timersRef.current.set(id, { ...timersRef.current.get(id), exit: exitTimer } as { auto: ReturnType<typeof setTimeout>; exit: ReturnType<typeof setTimeout> })
  }
